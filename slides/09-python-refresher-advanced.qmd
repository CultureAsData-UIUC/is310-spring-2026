...
If Application A needs version 1.0 of a library and Application B needs version 2.0, a virtual environment lets both coexist on your system.

## Before We Begin: Check pip

Ensure you have `pip` ("Python Installs Packages") installed:

```sh
pip3 --version
```

`pip` comes built-in to Python and is used to install external libraries.
...
Once created, you should see a `.venv` folder in your project directory.

When you open a new terminal in VS Code, you should see `(.venv)` in the prompt—this means your virtual environment is activated.

. . .

The status bar at the bottom left of VS Code will also show your active interpreter.

::: {.notes}
You can rename the .venv folder to something more descriptive like `is310-env` if you prefer.
:::

## Creating Virtual Environments in Terminal (Optional)

If you prefer the terminal, navigate to your project directory and run:
...
Now that your virtual environment is activated, install the `Rich` library:
...
::: {.notes}
You can also look in your virtual environment's `site-packages` directory to see installed libraries.
:::

## The Rich Library

`Rich` is a popular Python library for beautiful terminal formatting. It has nearly 50,000 GitHub stars and is used by hundreds of thousands of projects.

Let's see what it can do!

## Your First Rich Example

Create `rich_example.py`:
...
```sh
python3 rich_example.py
```

You'll see formatted text with colors and emojis!
...
console = Console()
movies = [
    {"Released": "Dec 20, 2019", "Title": "Star Wars: The Rise of Skywalker"},
    {"Released": "May 25, 2018", "Title": "Solo: A Star Wars Story"},
]

for movie in movies:
    console.print("\n[bold cyan]Movie Info:[/bold cyan]")
...
## Homework: CLI Data Entry Application

Now it's time to put everything together!

Create a Python script that:

1. Uses Rich to display sample data in a formatted way
2. Takes user input to add new entries
3. Confirms the data with the user
4. Saves the data to a file
5. Shows the file path where data was saved

::: {.notes}
This homework integrates virtual environments, packages (Rich), classes, file I/O, and control flow into one practical project.
::::: 
